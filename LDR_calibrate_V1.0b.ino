...
void DACdelay(byte channel, int output, byte retries)
{
  ...
  {
    if (output < 5) ldr_delay = 8000;
    else if (output < 20) ldr_delay = 5000;
    else if (output < 50) ldr_delay = 2500;
    else if (output < 100) ldr_delay = 1500;
    else if (output < 1000) ldr_delay = 1250;
    else if (output < 2000) ldr_delay = 500;
    else if (output < 3000) ldr_delay = 250;
    else ldr_delay = 200;
  }
  else
  {
    if (output < 10) ldr_delay = 1000;
    else if (output < 20) ldr_delay = 500;
    else if (output < 50) ldr_delay = 400;
    else if (output < 100) ldr_delay = 300;
    else if (output < 500) ldr_delay = 275;
    else if (output < 1000) ldr_delay = 250;
    else if (output < 2000) ldr_delay = 225;
    else ldr_delay = 200;
  }
...
long set_and_getResistance(byte channel, int output, byte retries)
{
  ...
  return measureResistance();
}

// Return the current resistance - uses 8x oversampling
  ...
  for (int step = 0; step < steps; step++)
  {
    ...
    if (quick) found = true;
      ...
      if ((((precision < -0.5) || (precision > 0.5)) && (step_target > 10000) && (retries < 3)) || ((((precision < -0.2) || (precision > 0.2)) && ((step_target <= 10000) && (step_target > 2500))) && (retries < 3)) || ((((precision < -0.1) || (precision > 0.1)) && (step_target < 2500)) && (retries < 3)))  {
        ...
        current_fine_adj = 1;
        setDAC(1, current_fine_adj);             // POSSIBLE IMPROVEMENT: Maybe this is not needed as it will be set when the bias adjustment happens
        setDAC(0, current_bias_adj);             // POSSIBLE IMPROVEMENT: Maybe this is not needed as it will be set when the bias adjustment happens
        DACdelay(0, current_bias_adj, retries);  // POSSIBLE IMPROVEMENT: Maybe this is not needed as it will be set when the bias adjustment happens
        step--;
      }
      else {
