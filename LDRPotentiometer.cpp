/*
  ...
  Created by Carsten Grønning, November 2, 2018.
...
void LDRPotentiometer::setInputRes(long _inputResAmp)
{
	inputResAmp = _inputResAmp;
}

long LDRPotentiometer::getInputRes()
{
  return inputResAmp;
}
...
void LDRPotentiometer::calcResistorsAtStep(byte _step)
{
  ...
  serieAtStep = max((1 - in_out) * size, (double)minResSerie);
  shuntAtStep = max(size * in_out, (double)minResShunt);
