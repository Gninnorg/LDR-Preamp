/*
  ...
  Fine adjustment is done by ModifiedExponentialSearch if (step_target > 300) and else by Binary Search
  Runtime approx. 450 seconds, precision approx. +2%/-2% - usually better
*/
...
//      Serial.print("Bias set to "); Serial.print(bias); Serial.print(" = "); Serial.print(maximum_with_set_bias); Serial.print(". Minimum value with this bias is "); Serial.println(minimum_with_set_bias);
    ...
    if (!(maximum_with_set_bias == step_target)) {
      ...
      if (step_target > 300) 
        index = ModifiedExponentialSearch(index, 4095, step_target, 1);
      else
        index = BinarySearch(index, 4095, step_target, 1);
    }
